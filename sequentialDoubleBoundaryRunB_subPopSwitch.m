function [controlFailureTrack1, controlFailureTrack2] = sequentialDoubleBoundaryRunB_subPopSwitch(folderName, fileName, pStruct, populationNum, populationLim)
...
%graphing concentration too
    ...
    concCalcB =((p.lambda_n .* Store.x(:, 1) + p.lambda_m .* Store.x(:, 2) + p.lambda_s .* Store.x(:, 3) + p.lambda_d .* Store.x(:, 4)) .* ((1-(p.kappa_threshold./p.kappa))))./(p.alphaB .* (Store.x(:, 1) + Store.x(:, 3)));
    ...
    for j=switchPoint1+1:switchPoint2
        refA = concCalcA(j);
        if refA < 0
            concA(j) = 0;
        elseif refA > p.uMaxA
    ...
    for k=switchPoint2+1:length(Store.t)
        concA(k) = p.uMaxA;
        concB(k) = p.uMaxB;
    end
