function dx = jointBoundaryEqn_constantProportionControls_withSwitch_try2(t, x, p, proportionConst)
...
elseif concCalcA > p.uMaxA
    ...
    if concCalcB < 0
        concB = 0;
        concHoldA = ((1 - p.kappa_threshold./p.kappa).*(p.lambda_n .* x(1) + p.lambda_m .* x(2) + p.lambda_s .* x(3) + p.lambda_d .* x(4)) - (p.alphaB.*concB.*(x(1)+x(3))))./(p.alphaA.*(x(1)+x(2)));
        if concHoldA < 0
            concA = 0;
        elseif concHoldA > p.uMaxA
            concA = p.uMaxA;
        else
            concA = concHoldA;
        end
