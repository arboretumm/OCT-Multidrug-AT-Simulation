function dx = controlACalcThreshold_controlBMax(t, x, p)
    dx = [0; 0; 0; 0; 0];

 concB = p.uMaxB;
 concCalcA = (((p.lambda_n .* x(1) + p.lambda_m .* x(2) + p.lambda_s .* x(3) + p.lambda_d .* x(4)) .* ((1-(p.kappa_threshold./p.kappa)))) - p.alphaB.*concB.*(x(1) + x(3)))./(p.alphaA .* (x(1) + x(2)));
