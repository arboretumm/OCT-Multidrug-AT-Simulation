function dx = controlAZero_controlBCalcThreshold(t, x, p)
    dx = [0; 0; 0; 0; 0];

concA = 0;
concCalcB = ((p.lambda_n .* x(1) + p.lambda_m .* x(2) + p.lambda_s .* x(3) + p.lambda_d .* x(4)) .* ((1-(p.kappa_threshold./p.kappa))))./(p.alphaB .* (x(1) + x(3)));
