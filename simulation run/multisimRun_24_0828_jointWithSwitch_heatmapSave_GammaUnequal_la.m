function multisimRun_24_0828_jointWithSwitch_heatmapSave_GammaUnequal_last(uMax, kR, folderBase)
tic
...
%equation parameters
    ...
    folderName = sprintf("%sjoint unequal/umax_%s_thresh_%s/", folderBase, num2str(uMax), num2str(kR));
    ...
    for i=1:length(ratio_2naught_3naught)
        for j=1:length(ratio_concA_concB)
            ...
            fileName = sprintf("individualGraphs/test_x2x3ratio%s_concAconcBratio%s_gammaUnequal", num2str(ratio_2naught_3naught(i)), num2str(ratio_concA_concB(j)));
            populationFileName = sprintf("%sindividualGraphs/population_x2x3ratio%s_concAconcBratio%s_gammaUnequal.mat", folderName, num2str(ratio_2naught_3naught(i)), num2str(ratio_concA_concB(j)));
            popStruct = fullEditPopulationParametersWithDrugLimits(alphaA, alphaB, gammaN, gammaA, gammaB, gammaD, kappa, kappa_threshold, 3000, naivePopIC, initCondA, initCondB, doubleMutPopIC, uMaxA, uMaxB);
            save(populationFileName, "popStruct");

            %results
            [failureTime4, failureTime5] = jjointDoubleBoundaryRun_constantProportionControls_last(folderName,fileName,popStruct, ratio_concA_concB(j));
