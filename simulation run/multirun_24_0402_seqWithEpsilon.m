function multirun_24_0402_seqWithEpsilon(uMax, kR)
tic
%population set up
...
folderName = sprintf("/Users/aftonwiddershins/Desktop/stuff/stuff/academic/phd/thesis/Mirror/thesis data/model data/double boundary shape exploration/2024_0415_sequentialSwitch_uMax_thresh_tests/umax_%s_threshold_%s/", num2str(uMax), num2str(kR));
...
for i=1:length(ratio_2naught_3naught)
    for j=1:length(ratio_growthRates)
        ...
        [failureTime4, failureTime5] = sequentialDoubleBoundaryRunA_concDrugSwitch(folderName, fileName, popStruct, altMaxA, altMaxB);
