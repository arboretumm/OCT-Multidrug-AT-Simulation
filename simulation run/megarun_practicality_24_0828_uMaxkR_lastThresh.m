function megarun_practicality_24_0828_uMaxkR_lastThresh()
    %uMaxVec = [20, 40, 80];
...
%    threshVec = [0.25, 0.5, 0.8];
   ...
   folderNameBase = "/Users/aftonwiddershins/Desktop/stuff/stuff/academic/phd/thesis/Mirror/thesis data/model data/double bound generation/last threshold tests/round 2/"; %finish with a / so that the multiruns work
   ...
   for j=1:length(threshVec)
       for i=1:coluMax
           ...
           multisimRun_24_0828_jointWithSwitch_heatmapSv_GammaUnequal_last(uMaxArr(j, i), threshVec(j), folderNameBase);
           multirun_24_0828_seqBWithEpsilonNoSwitch_GammaEqual_last(uMaxArr(j, i), threshVec(j), folderNameBase);
           multirun_24_0828_seqBWithEpsilonNoSwitch_GammaUnequal_last(uMaxArr(j, i), threshVec(j), folderNameBase);
           ...
           %run the practical version of the best
                ...
                %sequential practicality function
                    ...
                    jointEqThresh = practicalityAnalysisJoint_last(jointEqPopulationStruct, jointEqResultsFile, jointEqSaveFile, timeStep, jointEqConc, dosingIncrement);
                    jointUneqThresh = practicalityAnalysisJoint_last(jointUneqPopulationStruct, jointUneqResultsFile, jointUneqSaveFile, timeStep, jointUneqConc, dosingIncrement);
                    seqEqThresh = practicalityAnalysisSeq_last(seqEqPopulationStruct, seqEqResultsFile, seqEqSaveFile, timeStep, dosingIncrement);
                    seqUneqThresh = practicalityAnalysisSeq_last(seqUneqPopulationStruct, seqUneqResultsFile, seqUneqSaveFile, timeStep, dosingIncrement);
               ...
               %generate maximal treatment version
                    ...
                    eqMaxComboThresh = maximalTreatmentCombo_last(jointEqPopulationStruct, jointEqResultsFile, eqMaxComboSaveFile, maxDoseA, maxDoseB, timeStep);
                    uneqMaxComboThresh = maximalTreatmentCombo_last(jointUneqPopulationStruct, jointUneqResultsFile, uneqMaxComboSaveFile, maxDoseA, maxDoseB, timeStep);
                    eqMaxSeqThresh = maximalTreatmentSequential_last(jointEqPopulationStruct, jointEqResultsFile, eqMaxSeqSaveFile, maxDoseA, maxDoseB, timeStep);
                    uneqMaxSeqThresh = maximalTreatmentSequential_last(jointUneqPopulationStruct, jointUneqResultsFile, uneqMaxSeqSaveFile, maxDoseA, maxDoseB, timeStep);
