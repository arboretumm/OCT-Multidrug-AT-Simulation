function multisimRun_24_0329_tracking_sequential()
tic
...
folderName = sprintf("/Users/aftonwiddershins/Desktop/stuff/stuff/academic/phd/thesis/Mirror/thesis data/model data/double boundary shape exploration/2024_0329_sequentialRuns/umax_10_threshold_0.25/");
...
for i=1:length(ratio_2naught_3naught)
    ...
    for j=1:length(ratio_growthRates)
        for k=1:length(ratio_concA_concB)
            ...
            fileName = sprintf("individualGraphs/test_x2x3ratio_%d_gammaAgammaBratio_%d_concAconcBratio_%d_umax%d", ratio_2naught_3naught(i), ratio_growthRates(j), ratio_concA_concB(k), uMaxB); 
            ...
            [failureTime4, failureTime5] = sequentialDoubleBoundaryRunA_concDrugSwitch(folderName, fileName, popStruct, uMaxA, uMaxB);
    ...
    %graph heatmap
    figure('Visible', 'off');
    figureName = sprintf("Ratio of Simulated Failure and Ideal Failure for x(4) - Ratio of x(2) and x(3) %d", ratio_2naught_3naught(i));
    ...
    %graph heatmap
    figure('Visible', 'off');
    figureName = sprintf("Ratio of Simulated Failure and Ideal Failure for x(5) - Ratio of x(2) and x(3) %d", ratio_2naught_3naught(i));
