function megarun_OCTShapePlay_AandB_EqandUneq_last()
    ...
    folderNameBase = "/Users/aftonwiddershins/Desktop/stuff/stuff/academic/phd/thesis/Mirror/thesis data/model data/manuscript/oct regimen comparison/full pop last/"; %finish with a / so that the multiruns work
     ...
     for j=1:length(threshVec)
        for i=1:coluMax
            ...
            multirun_24_0926_seqPopSwitch_gammaEqual_last(uMaxArr(j, i), threshVec(j), folderNameBase);
            multirun_24_0926_seqPopSwitch_gammaUnequal_last(uMaxArr(j, i), threshVec(j), folderNameBase);
            %disp("normal Seq Pop")
            multirun_24_0926_seqPopSwitchB_gammaEqual_last(uMaxArr(j, i), threshVec(j), folderNameBase);
            multirun_24_0926_seqPopSwitchB_gammaUnequal_last(uMaxArr(j, i), threshVec(j), folderNameBase);
           ...
           [rowSeqBUneq, colSeqBUneq] = ind2sub(size(seqBUneqHeatmapVals), seqBUneqIndices);
            ...
            %run the max version?
            [failureTimeA, switchpoint] = multirun_maximalOCTShape_last(uMaxArr(j, i), threshVec(j));
            [failureTimeB, switchpoint2] = multirun_maximalOCTShape_Bfirst_last(uMaxArr(j, i), threshVec(j));
            [uneqFailureTimeA, switchpointu] = multirun_maximalOCTShape_uneq_last(uMaxArr(j, i), threshVec(j));
            [uneqFailureTimeB, switchpoint2u] = multirun_maximalOCTShape_Bfirst_uneq_last(uMaxArr(j, i), threshVec(j));
            ...
            %identify things to graph (rn 5 - joint best/worst, seq best/worst, max/max best)
                ...
                gammaEqComparisonArray((6*(length(threshVec)) + j), i) = multirun_elsaEqn_last(uMaxArr(j, i), threshVec(j));
                gammaEqComparisonArray((7*(length(threshVec)) + j), i) = multirun_uMaxA_uSingB_last(uMaxArr(j, i), threshVec(j));
                gammaEqComparisonArray((8*(length(threshVec)) + j), i) = multirun_uMaxB_uSingA_last(uMaxArr(j, i), threshVec(j));
                ...
                gammaUneqComparisonArray((6*(length(threshVec)) + j), i) = multirun_elsaEqn_uneq_last(uMaxArr(j, i), threshVec(j));
                gammaUneqComparisonArray((7*(length(threshVec)) + j), i) = multirun_uMaxA_uSingB_uneq_last(uMaxArr(j, i), threshVec(j));
                gammaUneqComparisonArray((8*(length(threshVec)) + j), i) = multirun_uMaxB_uSingA_uneq_last(uMaxArr(j, i), threshVec(j));
