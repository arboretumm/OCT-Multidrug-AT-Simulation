function megarun_OCTShapePlay()
    ...
    folderNameBase = "/Users/aftonwiddershins/Desktop/stuff/stuff/academic/phd/thesis/Mirror/thesis data/model data/double bound generation/manuscript figure generation/control shape comparison/first break 5050 only/"; %finish with a / so that the multiruns work
     ...
     for j=1:length(threshVec)
        for i=1:coluMax
            ...
            %run the max version?
            [failureTime, switchpoint] = multirun_maximalOCTShape(uMaxArr(j, i), threshVec(j));
            ...
            %identify things to graph (rn 5 - joint best/worst, seq best/worst, max/max best)
                ...
                gammaEqComparisonArray((4*(length(threshVec)) + j), i) = multirun_uMaxA_uSingB(uMaxArr(j, i), threshVec(j));
