function [failureTime, switchpoint] = multirun_maximalOCTShape_uneq(uMax, kR, folderBase)
    ...
    %         thresholdBreak = controlFailureTrack2;
        ...
        figureName = sprintf('%sindividualMaxMaxA_uneq/test_x2x3ratio%s_gammaUnequal.fig', folderName, num2str(ratio_2naught_3naught(q)));
        
        concBVecBest = concBVec;
        concBVecBest(ind:end) = uMaxB;
        ...
        options = odeset('RelTol', 1e-4, 'NonNegative', [1 2 3 4 5]);
        [t, x] = ode45(@(t, x) practicalityEquations(t, x, p, timeVector, concAVec, concBVecBest), tSPAN, x(end,:), options);
        Store.t=[Store.t ;t(2:end)];
        Store.x=[Store.x; x(2:end,:)];
        ...
        figureName2 = sprintf('%sindividualMaxMaxA_uneq/concentration_x2x3ratio%s_gammaEqual.fig', folderName, num2str(ratio_2naught_3naught(q)));
        
        gcf2 = figure('Visible', 'off');
        hold on;
        plot(timeVector, concAVec, 'LineWidth', 3)
        plot(timeVector, concBVecBest, 'LineWidth', 3)
