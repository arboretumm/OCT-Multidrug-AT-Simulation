function maxThreshBreak = multirun_elsaEqn_uneq(uMax, kR, folderBase)
    ...
    for q = 1:length(ratio_2naught_3naught)
        %establish population
         ...
         %pop struct & file names
            ...
            options = odeset('RelTol', 1e-4, 'NonNegative', [1 2 3 4 5]);
            [t, x] = ode45(@(t, x) elsasEquations(t, x, p), tSPAN, x(end,:), options);
            Store.t=[Store.t ;t(2:end)];
            Store.x=[Store.x; x(2:end,:)];

            figureName = sprintf('%sindividualElsaUnequal/test_x2x3ratio%s_gammaUnequal.fig', folderName, num2str(ratio_2naught_3naught(q)));
           ...
           concCalcBStart = (p.lambda_n.*p.kappa_threshold.*(1 - p.kappa_threshold./p.kappa))./(2.*p.alphaB.*(Store.x(:, 1)+Store.x(:, 2)));
            %concCalcAStart = (((p.lambda_n .* Store.x(:, 1) + p.lambda_m .* Store.x(:, 2) + p.lambda_s .* Store.x(:, 3) + p.lambda_d .* Store.x(:, 4)) .* ((1-(p.kappa_threshold./p.kappa)))) - p.alphaB.*concCalcBStart.*(Store.x(:, 1) + Store.x(:, 3)))/(p.alphaA .* (Store.x(:, 1) + Store.x(:, 2)));
            
            concA = zeros(1, length(concCalcBStart));
            concB = zeros(1, length(concCalcBStart));

        for i = 1:length(concCalcBStart)
            concCalcB = concCalcBStart(i);
            concCalcA = (((p.lambda_n .* Store.x(:, 1) + p.lambda_m .* Store.x(:, 2) + p.lambda_s .* Store.x(:, 3) + p.lambda_d .* Store.x(:, 4)) .* ((1-(p.kappa_threshold./p.kappa)))) - p.alphaB.*concCalcB.*(Store.x(:, 1) + Store.x(:, 3)))/(p.alphaA .* (Store.x(:, 1) + Store.x(:, 2)));
            if concCalcA < 0
                concA(1, i) = 0;
                if concCalcB < 0
                    concB(1, i) = 0;
            ...
            elseif concCalcA > p.uMaxA
                ...
                reconcA = (((p.lambda_n .* Store.x(i, 1) + p.lambda_m .* Store.x(i, 2) + p.lambda_s .* Store.x(i, 3) + p.lambda_d .* Store.x(i, 4)) .* ((1-(p.kappa_threshold./p.kappa)))) - p.alphaB.*concB(1, i).*(Store.x(i, 1) + Store.x(i, 3)))/(p.alphaA .* (Store.x(i, 1) + Store.x(i, 2)));
            ...
            figureName2 = sprintf('%sindividualElsaUnequal/concentration_x2x3ratio%s_gammaUnequal.fig', folderName, num2str(ratio_2naught_3naught(q)));
