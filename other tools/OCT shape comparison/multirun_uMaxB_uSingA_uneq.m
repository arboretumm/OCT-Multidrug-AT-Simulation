function maxThreshBreak = multirun_uMaxB_uSingA_uneq(uMax, kR, folderBase)
        ...
        %establish population
         ...
         %pop struct & file names
            ...
            if t(end) < tSPAN(end)
                ...
                [t, x] = ode45(@(t, x) controlACalcThreshold_controlBMax(t, x, p), [t(end) tSPAN(end)], x(end,:), options);
            ...
            figureName = sprintf('%sindividualuMaxBuSingAuneq/test_x2x3ratio%s_gammaEqual.fig', folderName, num2str(ratio_2naught_3naught(q)));
            ...
            concB = ones(length(Store.t))*p.uMaxB;
            concCalcA = (((p.lambda_n .* Store.x(:,1) + p.lambda_m .* Store.x(:,2) + p.lambda_s .* Store.x(:,3) + p.lambda_d .* Store.x(:,4)) .* ((1-(p.kappa_threshold./p.kappa)))) - p.alphaB.*p.uMaxB.*(Store.x(:,1) + Store.x(:,3)))./(p.alphaA .* (Store.x(:,1) + Store.x(:,2)));
            concCalcA = concCalcA(:, 1)';
            concA = zeros(1, length(Store.t));

            for i = 1:length(concCalcA)
                refA = concCalcA(1, i);
                if refA < 0
                    concA(1, i) = 0;
                elseif refA > p.uMaxA
            ...
            concA(1, timeSwitch+1:end) = p.uMaxA;
            ...
            figureName2 = sprintf('%sindividualuMaxBuSingAuneq/concentration_x2x3ratio%s_gammaUnequal.fig', folderName, num2str(ratio_2naught_3naught(q)));
