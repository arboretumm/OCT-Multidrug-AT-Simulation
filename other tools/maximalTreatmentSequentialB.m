function [thresholdBreak] = maximalTreatmentSequentialB(popStructFile, runFile, saveFile, drugConcA, drugConcB, timestep)
    ...
    %find minimum value
        optionsFind = odeset('Events', @(t, x) eventFindTreatmentMinima_PracticalityA(t, x, p, timeVector, concZeroVec, concBVec), 'RelTol', 1e-4, 'NonNegative', [1 2 3 4 5], 'Refine', 10);
        [t2, x2, te_a, xe_a, ie_a] = ode45(@(t, x) practicalityEquations(t, x, p, timeVector, concZeroVec, concBVec), tSPAN, x(end,:), optionsFind);
        if length(te_a) ~= 0
            if xMin > xe_a(5)
                xMin = xe_a(5);
            end
    ...
    %maintain regimen until 20% increase for RECIST
        ...
        [t, x] = ode45(@(t, x) practicalityEquations(t, x, p, timeVector, concZeroVec, concBVec), tSPAN, x(end,:), options);
        Store.t=[Store.t ;t(2:end)];
        Store.x=[Store.x; x(2:end,:)];
        if t(end) < tSPAN(end)
            options = odeset('RelTol', 1e-4, 'NonNegative', [1 2 3 4 5]);
            [t, x] = ode45(@(t, x) practicalityEquations(t, x, p, timeVector, concAVec, concZeroVec), [t(end) tSPAN(end)], x(end,:), options);
            Store.t=[Store.t ;t(2:end)];
            Store.x=[Store.x; x(2:end,:)];
        end
