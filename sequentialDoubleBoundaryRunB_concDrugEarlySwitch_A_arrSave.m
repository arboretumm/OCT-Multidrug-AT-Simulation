function [controlFailureTrack1, controlFailureTrack2] = sequentialDoubleBoundaryRunB_concDrugEarlySwitch_A_arrSave(folderName, fileName, pStruct, drugConc1, drugConc2)
...
%GENERAL RULES
    ...
    if initialDrugBVal < drugConc2
        ...
        Store.t=[Store.t ;t(2:end-1)];
        Store.x=[Store.x; x(2:end-1,:)];
        ...
        %sprintf("phase 1 done")
            if t(end) < tSPAN(end)
                ...
                [t, x] = ode45(@(t, x) controlACalcThreshold_controlBMax(t, x, p), [t(end-1) tSPAN(end)], x(end-1,:), options2);
                xHold2 = x(2:end-1,:);
                Store.t=[Store.t ;t(2:end-1)];
                Store.x=[Store.x; x(2:end-1,:)];
                lengthVal = length(t) - 2;
                ...
                if t(end) < tSPAN(end)
                    ...
                    [t, x] = ode45(@(t, x) controlAMax_controlBMax(t, x, p), [t(end-1) tSPAN(end)], x(end-1,:), options3);
